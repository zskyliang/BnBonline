...
func _run() -> void:
	...
	_test_catalog_and_team_tint_materials()
...
func _test_catalog_and_team_tint_materials() -> void:
	...
	for definition: CharacterDefinition in definitions:
		...
		_check(
			not definition.team_tint_material_names.is_empty(),
			"%s declares local team-tint material names" % definition.id
		)
		if scene == null:
			continue
		...
		for material_name: String in definition.team_tint_material_names:
			matched = matched or available_names.has(material_name)
		_check(matched, "%s team-tint whitelist matches imported materials" % definition.id)
		_check(
			definition.model_path.ends_with("%s.glb" % definition.id),
			"%s uses its approved original storybook GLB" % definition.id
		)
		_check(
			not definition.place_bubble_animation_aliases.is_empty() \
				and not definition.trapped_animation_aliases.is_empty() \
				and not definition.defeat_animation_aliases.is_empty() \
				and not definition.victory_animation_aliases.is_empty(),
			"%s declares all four non-locomotion action aliases" % definition.id
		)
		instance.free()
...
func _test_storybook_material_constraints() -> void:
	...
	_check(material.roughness >= 0.86, "storybook paint material remains matte")
	_check(material.metallic == 0.0, "storybook paint material is non-metallic")
	_check(
		material.normal_enabled \
			and material.diffuse_mode == BaseMaterial3D.DIFFUSE_TOON,
		"storybook material combines paper grain with Compatibility toon shading"
	)
	var outline := material.next_pass as StandardMaterial3D
	_check(
		outline != null \
			and outline.cull_mode == BaseMaterial3D.CULL_FRONT \
			and outline.grow,
		"character material uses a front-culled expanded ink shell"
	)
...
func _test_character_palette_views() -> void:
	...
	var team_tint_found: bool = _has_surface_color(
		actor_view,
		CharacterCatalog.get_definition("cat").team_tint_material_names,
		PaintPalette.get_color("purple")
	)
	_check(team_tint_found, "cat local team surfaces receive selected purple palette")
	for action: StringName in [
		&"Idle", &"Waddle", &"PlaceBubble", &"Trapped", &"Defeat", &"Victory"
	]:
		_check(actor_view.has_action(action), "cat imports %s animation" % action)
	_check(
		actor_view.play_action(&"PlaceBubble") \
			and actor_view.get_current_action() == &"PlaceBubble",
		"placing a bubble triggers a visual-only one-shot action"
	)
	actor.stats.is_trapped = true
	actor_view.call("_process", 0.14)
	_check(
		actor_view.get_current_action() == &"Trapped",
		"trapped logic state overrides the one-shot action"
	)
...
func _test_item_views_and_camera() -> void:
	...
	for item_type: int in ArenaItemType.ALL:
		...
		_check(
			...
			"%s pickup uses imported storybook model and identification ring"
	...
	_check(
		...
		"runtime ignores legacy angles and clamps zoom to 135%"
	)
	...
	_check(
		...
		"zoom reset preserves the fixed -30/42 camera"
	)
	arena.reset_camera()
	_check(
		arena.get_azimuth() == MatchSettings.DEFAULT_CAMERA_AZIMUTH \
	...
	_check(
		...
		"zoom changes persist when the adjustment finishes"
	)
	arena.queue_free()
	root_3d.queue_free()
	await process_frame
