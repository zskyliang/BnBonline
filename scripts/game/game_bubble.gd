...
func setup(new_owner: GameActor, new_cell: Vector2i, skin: String) -> void:
	...
	_fuse_timer.wait_time = GameConstants.BUBBLE_FUSE_SECONDS
	_fuse_timer.timeout.connect(explode_now)
	add_child(_fuse_timer)
	_fuse_timer.start()
	explode_at_ms = Time.get_ticks_msec() + int(GameConstants.BUBBLE_FUSE_SECONDS * 1000.0)
